...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        if class_mode not in {'categorical', 'binary', 'sparse',
                              'input', 'multi_categorical', None}:
            raise ValueError('Invalid class_mode:', class_mode,
                             '; expected one of "categorical", '
                             '"binary", "sparse", "input", "multi_categorical"'
                             ' or None.')
        ...
        if self.class_mode == 'multi_categorical':
            for multilabel in multilabel_classes:
        ...
        if self.class_mode == 'multi_categorical':
            self.num_multilabel_classes = len(multilabel_classes)
            self.multilabel_class_indices = []
            for key in self.class_indices.keys():
                if key in multilabel_classes:
                    self.multilabel_class_indices.append([self.class_indices[key], key])
            self.multilabel_class_indices = dict(self.multilabel_class_indices)
